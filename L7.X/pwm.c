...
void pwm_init(){
  ...
  CCPR1L = 0;      //inicialmente duty cycle 0, motor desligado.
  CCP1CONbits.DC1B = 0;                      
  //Configuraçãp canal 2
  CCP2CONbits.CCP2M = 0b1100;
  CCPR2L = 0;    
  DC2B0 = 0;  //duty cycle inicialmente 0       
  DC2B1 = 0;
...
/// @param channel - canal do PWM (1: motor esquerdo; 2: motor direito)
...
void pwm_set(int channel, int duty_cycle){
  ...
  }   //CCP1CONbits.DC1B não precisa ser programado pois devido à resolução de 10 bits duty cicle de 100% corresponderá a n~1000
  else if(channel == 2){
    CCPR2L = duty_cycle >> 2;  //8 bits mais significativos do duty_cycle   
  }   //pelo mensmo motivo 8 bits menos significativos podem ser desprezados
}
